...
The `public contributions` and `private contributions` by users in Japan on `2022/4/8 1:45 AM UTC`. This list contains users from `Japan` and cities `Tokyo`.

There are `1 countries` and `1 cities` can be found [here](https://github.com/yusukebe/top-github-users-only-japan).

There are `90 users`  in Japan. You need at least `824 followers` to be on this list.
